...
router.get('/user', function (req, res, next) {
  User.find({phone: req.phone}, (err, doc) => {
    if(err) {
      res.status(500).send({
        code: 1,
        message: '获取用户信息失败'
    ...
    if(doc.length > 0) {
      res.json({
        code: 0,
        data: {
          user: doc[0] // TODO: filter of props
        }
      })
      return
    } else {
...
router.post('/signup', function (req, res, next) {
  ...
  User.find({ phone: logName }, (err, doc) => {
    if (err) {
      ...
      User.create({ phone: logName, password, role }, (err, doc) => {
        if (err) {
          res.status(500).send({
            code: 1,
            message: err.message
...
router.post('/login', (req, res, next) => {
  ...
  User.findOne({ phone: logName, password }, (err, doc) => {
    if (err) {
      ...
      let content = { phone: logName }// 要生成token的主题信息
      // 这是加密的 key（密钥）
